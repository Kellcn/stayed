...
class File
{
    ...
    public function log($intLogLevel, $strLog, $strFile, $intLine, $intLogId)
    {
        ...
        $strLogType = 'NOTICE';
        //include "../Log.php";
        if (isset(\Local\Log::$arrLogNames[$intLogLevel])) {
            $strLogType = \Local\Log::$arrLogNames[$intLogLevel];
        }
        $str = $strLogType . $this->_strSep . $intLogId . $this->_strSep . date('Ymd H:i:s') . $this->_strSep . $strFile . ':' . $intLine . $this->_strSep . $strLog . $this->_strSepOfLine;
        if (($intLogLevel & \Local\Log::LOG_WARNING) || ($intLogLevel & \Local\Log::LOG_FATAL)) {
