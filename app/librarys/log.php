...
class log
{
        ...
        if (! empty($arrConfig)) {
            foreach ($arrConfig as $_strModule => $_arrConf) {
                if (isset($_arrConf['file'])) {
                    $_intLevel = 0xFF;
                    if (isset($_arrConf['level'])) {
                        $_intLevel = intval($_arrConf['level']);
                    }
                    include_once "Logfile/File.php";
                    self::$_arrLogs[$_strModule] = new \Local\Logfile\File($_arrConf['file'], $_intLevel);
                }
            }
        }
    ...
    public static function getModule($strModule = '')
    {
        ...
        if (empty($strModule))
            $strModule = self::$_strDefaultModule;
